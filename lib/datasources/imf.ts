 ...
 * Fetches data from IMF SDMX API
 */
...
export async function fetchIMFSeries(
  ...
  const url = `https://dataservices.imf.org/REST/SDMX_JSON/data/${flow}/${key}?format=sdmx-json`

  const response = await withRetry(async () => {
    const res = await fetchWithTimeout(url)
    if (!res.ok) {
      throw new Error(`IMF ${res.status}: ${res.statusText} - ${url}`)
    }
    return res
  })
