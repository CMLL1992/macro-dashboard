...
export async function getJobStatus(jobName: string): Promise<JobStatus | null> {
  ...
  if (isUsingTurso()) {
    ...
    const row = db.prepare('SELECT * FROM job_status WHERE job_name = ?').get(jobName) as any
    return row || null
  }
}
...
export async function getAllJobStatuses(): Promise<JobStatus[]> {
  try {
    ...
    if (isUsingTurso()) {
      ...
      const rows = db.prepare('SELECT * FROM job_status ORDER BY job_name').all() as any[]
      return rows
    }
  } catch (error) {
