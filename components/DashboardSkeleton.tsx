...
export function RegimeSkeleton() {
  return (
    <section className="rounded-lg border bg-card p-6">
      ...
      <div className="h-4 w-96 bg-muted rounded animate-pulse mb-4" />
      <div className="flex flex-wrap items-center gap-2 mb-2">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-8 w-32 bg-muted rounded-full animate-pulse" />
        ))}
      </div>
      <div className="h-4 w-80 bg-muted rounded animate-pulse mb-3" />
      <div className="flex flex-wrap items-center gap-3">
        {[1, 2].map((i) => (
          <div key={i} className="h-6 w-48 bg-muted rounded-full animate-pulse" />
        ))}
      </div>
    </section>
  )
      ...
      <div className="space-y-3">
        {[1, 2].map((i) => (
          <div key={i} className="border rounded p-3 bg-muted">
            <div className="h-5 w-64 bg-muted rounded animate-pulse mb-1" />
