...
export default function AdminDashboardPage() {
  ...
  const loadData = async () => {
    try {
      const [statusRes, queueRes, newsRes, eventsRes] = await Promise.all([
        fetch('/api/notifications/status', { cache: 'no-store' }),
        fetch('/api/notifications/queue/status', { cache: 'no-store' }),
          ...
          <div className="rounded-lg border bg-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-lg border bg-muted p-4">
                <div className="text-sm text-muted-foreground mb-1">Mensajes en Cola</div>
                <div className="text-2xl font-bold">
                  {queueStatus.queueLength}
                </div>
              </div>
              <div className="rounded-lg border bg-muted p-4">
                <div className="text-sm text-muted-foreground mb-1">Rate Limit Global</div>
                <div className="text-lg font-bold">
                  {queueStatus.globalRateLimit.used} / {queueStatus.globalRateLimit.limit} msg/min
                </div>
                {queueStatus.globalRateLimit.waitTime > 0 && (
                  <div className="text-xs text-yellow-600 mt-1">
                    Espera: {Math.ceil(queueStatus.globalRateLimit.waitTime / 1000)}s
                  </div>
                )}
              </div>
              <div className="rounded-lg border bg-muted p-4">
                <div className="text-sm text-muted-foreground mb-1">Rate Limit por Chat</div>
                {queueStatus.chatRateLimits.length > 0 ? (
                  queueStatus.chatRateLimits.map((chat, idx) => (
                    <div key={idx} className="text-sm">
                      <div className="font-medium">
                        {chat.used} / {chat.limit} msg/min
                      </div>
                      {chat.waitTime > 0 && (
                        <div className="text-xs text-yellow-600">
                          Espera: {Math.ceil(chat.waitTime / 1000)}s
                        </div>
                      )}
                    </div>
                  ))
