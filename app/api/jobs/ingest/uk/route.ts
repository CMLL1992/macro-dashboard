...
export async function POST(request: NextRequest) {
  ...
  try {
    logger.info('Starting UK indicators ingestion', { job: jobId, dryRun })
    ...
    for (const indicator of indicators) {
      try {
        ...
        // UK indicators: prefer ONS (official source, no API key), fallback to Trading Economics
        if (indicator.source === 'ons' || indicator.source === 'trading_economics') {
          // Try ONS first (no API key required, official source)
          try {
            const { fetchONSIndicator } = await import('@/lib/ingestors/onsUK')
            const observations = await fetchONSIndicator(indicator.id)
            ...
            if (observations && observations.length > 0) {
              ...
              sourceUsed = 'ons'
              logger.info(`Successfully fetched ${indicator.id} from ONS`, {
                job: jobId,
            ...
            const errorMsg = onsError instanceof Error ? onsError.message : String(onsError)
            
            // Classify error type
            let errorType = 'source_mapping_error'
            if (errorMsg.includes('404') || errorMsg.includes('Not Found') || errorMsg.includes('No ONS mapping')) {
              errorType = 'source_mapping_error'
            } else if (errorMsg.includes('timeout') || errorMsg.includes('5')) {
              errorType = 'source_down'
            }
            
            logger.warn(`ONS fetch failed for ${indicator.id}, will try Trading Economics fallback`, {
              job: jobId,
              indicatorId: indicator.id,
              error: errorMsg,
              errorType,
          ...
          // Fallback to Trading Economics if ONS didn't work or if explicitly requested
          if (!macroSeries && indicator.source === 'trading_economics') {
            ...
            try {
              ...
              if (errorMessage.includes('403') || errorMessage.includes('No Access to this country')) {
                ...
                ingestErrors.push({ 
                  indicatorId: indicator.id, 
                  error: `source_not_allowed: Trading Economics plan does not include UK data`,
        ...
        logger.error(`Failed to ingest UK indicator: ${indicator.id}`, { 
          job: jobId, 
          indicatorId: indicator.id, 
          error: errorMessage,
          errorType,
