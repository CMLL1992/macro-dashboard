/**
 * Job: Ingest Switzerland economic indicators into SQLite
 * POST /api/jobs/ingest/ch
 * Protected by CRON_TOKEN
 * 
 * Ingests all Switzerland indicators (CHF) used by the dashboard
 */
...
// Load CH indicators config
function loadCHIndicators() {
  try {
    const configPath = path.join(process.cwd(), 'config', 'ch-indicators.json')
    const raw = fs.readFileSync(configPath, 'utf8')
    return JSON.parse(raw)
  } catch (error) {
    logger.error('Failed to load ch-indicators.json', { error })
    return { indicators: [] }
  }
}

const CH_INDICATORS = loadCHIndicators()
